...
def run_bot(device):
    ...
    while True:
        ...
        # # Priority 2: find cp
        # pos = find_image(gray, "cp.png")
        # if pos:
        #     x, y = pos
        #     # region to the right of image
        #     region = img[y-10:y+80, x+50:x+500] # adjust until only numbers visible

        #     # optional: improve OCR
        #     region = cv2.resize(region, None, fx=4, fy=4, interpolation=cv2.INTER_CUBIC)
            
        #     gray = cv2.cvtColor(region, cv2.COLOR_BGR2GRAY)
        #     results = reader.readtext(gray, allowlist="01234567890TABM ")
